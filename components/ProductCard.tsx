...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onClick, onAddToCart, bestSeller, compactOnMobile }) => {
  ...
  return (
    <article
      className={`group relative flex flex-col h-full bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl hover:shadow-slate-200/60 hover:-translate-y-1 hover:border-slate-200 ${compactOnMobile ? 'w-[95%] md:w-full mx-auto md:mx-0' : ''}`}
      onClick={() => onClick(product)}
    >
      {/* Image block - 10% shorter on mobile when compact */}
      ...
      {/* Content - tighter padding/spacing on mobile when compact */}
      <div className={`flex flex-col flex-grow ${compactOnMobile ? 'p-3 md:p-5' : 'p-4 md:p-5'}`}>
        ...
        {/* Price + CTA row */}
        <div className={`mt-auto flex flex-col ${compactOnMobile ? 'gap-2 md:gap-3' : 'gap-3'}`}>
